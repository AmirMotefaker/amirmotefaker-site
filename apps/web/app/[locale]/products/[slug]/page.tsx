...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  const keywords = fa
    ? [displayName, `${displayName} چیست`, product.category, "امیر متفکر", "محصول فناوری", "محصول دیجیتال", product.shortDescriptionFa]
    : [displayName, `${displayName} product`, product.category, "Amir Motefaker", "technology product", "digital product", product.positioning];
...
export default async function Page({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const productSchema = {
    ...
    keywords: [product.category, product.positioning, displayName],
  };
